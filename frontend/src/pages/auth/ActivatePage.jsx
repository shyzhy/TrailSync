...
import { Spinner, SuccessSeal } from '../../components/ui/index.js';
import ResendEmailButton from '../../components/auth/ResendEmailButton.jsx';
import { FONT_SERIF } from '../../styles/fonts.js';
import { saveSession, STUDENT_LOGIN_PATH } from '../../lib/auth.js';
...
// Where the confirmation email leads. The page POSTs the token, since mail scanners open every link with a GET.
export default function ActivatePage() {
  const [state, setState] = useState({ phase: 'working' });
  const ran = useRef(false);

  useEffect(() => {
    // React's development double-mount would otherwise post twice.
    ...
    (async () => {
      try {
        ...
        if (res.ok && data.access) {
          // Not "remember me": this could be a shared school computer, and onboarding saves every step.
...
// For a link too broken to say whose account it was: ask for the email and send a new one.
