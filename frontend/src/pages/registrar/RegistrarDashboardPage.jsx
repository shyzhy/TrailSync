...
// Today's pickups read in claim terms: at the counter the only question is whether the person has turned up.
...
export default function RegistrarDashboardPage() {
  ...
  // Loads on its own, so a slow or failed month doesn't hold up the rest of the dashboard.
  const loadCalendar = useCallback(async () => {
    ...
    try {
      ...
      // Blank days would read as "no pickups", so say plainly that the numbers didn't load.
      setCalendarError(toApiError(error));
    }
  }, [calendarMonth]);

  // Loads on its own so neither failure can hide the other. Staff-only: duplicate_flag is on no student-facing serializer.
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // A 403 means a non-staff or unapproved account; ErrorState says so instead of drawing a half-empty dashboard.
  ...
  return (
    <div className="ts-app-shell lg:flex" style={FONT_SANS}>
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-6 py-8 sm:py-10">
        ...
        {/* Newest requests, release calendar and today's pickups */}
        <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-5">
          ...
          <div className="lg:col-span-2">
            {/* Picking a day opens it on the full calendar. */}
            <div className="flex items-center justify-between">
              <h2 className="ts-ink text-lg font-semibold" style={FONT_SERIF}>
                Release calendar
              </h2>
