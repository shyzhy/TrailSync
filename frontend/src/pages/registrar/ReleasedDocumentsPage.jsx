...
// The current month, which is what the office usually looks for.
function thisMonth() {
  const now = new Date();
...
export default function ReleasedDocumentsPage() {
  ...
  // The filters in one place, so the table and the export never disagree.
  ...
  // Fetched rather than linked (the endpoint needs the Authorization header), then saved through a throwaway anchor so no popup blocker interferes.
  ...
  return (
    <div className="ts-app-shell lg:flex" style={FONT_SANS}>
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-6 py-8 sm:py-10">
        ...
        {status === 'ready' && rows.length > 0 && (
          <>
            {/* A table on wide screens; the same rows as cards on a phone, where eight columns can't be read side by side. */}
