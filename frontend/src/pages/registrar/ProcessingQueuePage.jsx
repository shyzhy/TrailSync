...
// A review is a page of its own, so it can be linked, reloaded and shared.
  ...
  return (
    <div className="ts-app-shell lg:flex" style={FONT_SANS}>
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-6 py-8 sm:py-10">
        ...
        {/* Labelled filters, since a placeholder disappears the moment someone types. */}
        ...
        {/* Replaces the table, since an empty one would read as "nothing waiting". */}
        {status === 'error' && (
        ...
        <div className="ts-card mt-6 overflow-hidden">
          ...
          {status === 'loading' && (
            <SkeletonGroup label="Loading requests">
              {/* Column spans match the real table's, so rows don't jump when the data arrives. */}
          ...
          {status === 'ready' &&
            results.map((r) => (
              ...
              >
                ...
                {/* Missing requirements ride alongside the stage: a request can be ready for pickup and still have had a missing upload. */}
