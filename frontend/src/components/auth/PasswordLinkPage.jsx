...
import { GlassScene } from '../layout/GlassScene.jsx';
import { BusyLabel, PasswordField, Spinner } from '../ui/index.js';
import { AuthHeader, BackToLogin, LOGIN_FOR } from './ResetLink.jsx';
...
// Expired and used links are one case: either way, the person needs a new link.
...
// A page reached from an emailed link that lets someone choose a password: checks the link on arrival, then logs them out to their login.
export default function PasswordLinkPage({ audience, validatePath, confirmPath, flash, copy, renderDead, renderIntro }) {
  ...
  const check = useCallback(async () => {
    ...
    setPhase('checking');
    try {
      const res = await post(validatePath, { uid, token });
      if (res.ok) {
  ...
  const submit = async (e) => {
    ...
    setSaving(true);
    try {
      const res = await post(confirmPath, { uid, token, new_password: newPassword, confirm_new_password: confirmPassword });
      if (!res.ok) throw await errorFromResponse(res);
      const data = await res.json();
      // Any session this browser still holds belongs to someone who hasn't logged in with this password.
      clearSession();
      setFlash(flash);
      window.location.replace(LOGIN_FOR[data.login] || LOGIN_FOR[audience]);
    } catch (error) {
      const apiError = toApiError(error);
      // The link died while the form was open: that's the dead-link page, not a form error.
      ...
      <div className="mt-8" aria-live="polite">
        {phase === 'checking' && (
          <div role="status" className="flex items-center gap-3">
            ...
            <p className="ts-ink text-lg font-medium">{copy.checking}</p>
          </div>
        )}
        ...
        {phase === 'form' && (
          <>
            ...
            {renderIntro ? renderIntro(linkInfo) : <p className="ts-soft mt-2 text-sm leading-relaxed">{copy.formIntro}</p>}
