...
// Ask for a reset link. The server answers the same for every address, so nothing reveals whether an account exists.
export function ResetLinkRequestForm({ autoFocus = false, onSent, idPrefix = 'reset' }) {
  ...
  const submit = async (e) => {
    ...
    setSending(true);
    try {
      // A plain fetch: nobody is logged in here, and a stale token must not be sent along.
