 ...
 *
 * "Futuristic" here comes from how the page moves - a photo that breathes,
 * a light that drifts across it, sections that rise into place, a request
 * that visibly travels from Request to Release - and NOT from a new palette.
 * It is the same paper, institutional blue and gold, the same serif, and
 * the same glossy raised buttons as every other TrailSync screen, so the
 * step from here to the login feels like walking further into one building.
 *
 * Every piece of motion stops for anyone with reduced motion turned on.
 */

// What the office can actually be reached by, in one place. Only details
// this system already states elsewhere are listed; there is no phone number
// or email on record here to publish.
const OFFICE = {
...
// The example ticket in the hero walks the same stages a student sees.
 ...
 *
 * Marks every [data-reveal] element once it is on screen and never
 * un-marks it, so scrolling back up doesn't replay the entrance. Without
 * IntersectionObserver - or with reduced motion on - everything is simply
 * shown, because hidden-until-animated content must never stay hidden.
 */
...
/** A small, looping stand-in for a real ticket, so the hero shows the product moving. */
...
function Lifecycle() {
  ...
  // Walks the four steps on its own while the section is on screen, and
  // stops for good the moment someone picks a step themselves.
  useEffect(() => {
    ...
    <section ref={sectionRef} id="how" className="ts-land-section scroll-mt-20 px-6 py-20 sm:py-28">
      <div className="mx-auto max-w-5xl">
        ...
        <div data-reveal style={{ '--reveal-delay': '120ms' }} className="ts-land-track mt-14">
          {/* The rail, the part of it already travelled, and the ticket
              moving along it. */}
...
export default function LandingPage() {
  ...
  // Smooth in-page scrolling for the anchor links, only on this page and
  // only for people who haven't asked for less motion.
  useEffect(() => {
    if (prefersReducedMotion()) return undefined;
  ...
  // Parallax: the photo drifts at a quarter of the scroll speed. Written to a
  // CSS variable from one rAF per frame rather than React state, so scrolling
  // never re-renders the page. The nav's glass switches on past the fold edge.
  useEffect(() => {
  ...
  return (
    <div ref={rootRef} className="ts-landing" style={FONT_SANS}>
      ...
      {/* A sibling of everything that animates: a transformed ancestor would
          stop this bar being fixed to the viewport. */}
      <header className={`ts-land-nav ${scrolled ? 'is-scrolled' : ''}`}>
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-5 py-3 sm:px-8">
          ...
          <nav className="flex items-center gap-1 sm:gap-2" aria-label="Site">
            ...
            {/* Staff know where they are going; this stays small on purpose. */}
...
const LANDING_CSS = `
  ...
  /* ---- Hero ----------------------------------------------------------
     Three layers over one photo: the photo itself (slow zoom + parallax),
     a drifting light in institutional blue and gold, and a fade into the
     paper below so the page continues rather than stopping at a band. */
  ...
  /* The content sits on the same frosted paper as the login pages. */
  .ts-land-hero-card {
    position: relative;
    padding: 2rem 1.5rem;
    border-radius: 22px;
